...
router.route('/switch/:direction')
    .get(function(req, res) {
		...
		var pinFwd = new onoff.Gpio(stepPinForward, 'out');
		var pinRev = new onoff.Gpio(stepPinReverse, 'out');
		var pinPow = new onoff.Gpio(powerPin, 'out');
		...
		pinFwd.writeSync(writeFwdValue);
		pinRev.writeSync(writeRevValue);
		pinPow.writeSync(1);
		setTimeout(function(){
...
{
	console.log('gpioWrite: ' + pin + " = " + value);
	
	gpio.write(pin, value, function(err) {
		if (err)
			console.log('Fel fel fel vid skrivning till pin' + pin);
        else
